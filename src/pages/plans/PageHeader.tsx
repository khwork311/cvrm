...
export const PageHeader = ({ search, setSearch, statusFilter, setStatusFilter }: PageHeaderProps) => {
  ...
  return (
    <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl dark:text-white/90">{t('plans:title')}</h1>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{t('plans:subtitle')}</p>
        </div>
        <Link to="/plans/create" title={t('plans:createNewPlan')}>
