...
import { addressesApi, countriesApi, type AddressFilters, type City, type Country } from '../api/companies.api';
...
export function useAddresses(companyId: number | null, filters?: AddressFilters) {
  ...
  return useSWR(key, () => addressesApi.getAll(companyId!, filters), {
...
export function useAddress(id: number | null) {
  return useSWR(id ? ['address', id] : null, () => addressesApi.getById(id!), {
    revalidateOnFocus: false,
  });
}
...
export function useCreateAddress(companyId: number) {
  return useSWRMutation(['addresses', companyId], async (_key, { arg }: { arg: AddressFormData }) => {
...
export function useUpdateAddress() {
  return useSWRMutation('addresses', async (_key, { arg }: { arg: { id: number; data: AddressFormData } }) => {
    // Convert string IDs to numbers for API
    const addressData = {
      ...arg.data,
...
export function useToggleAddressStatus() {
  return useSWRMutation('addresses', async (_key, { arg }: { arg: number }) => {
...
export function useCountries() {
  return useSWR<Country[]>('countries', () => countriesApi.getAll(), {
    revalidateOnFocus: false,
  });
}
...
export function useCities(countryId: number | null) {
  return useSWR<City[]>(countryId ? ['cities', countryId] : null, () => countriesApi.getCities(countryId!), {
    revalidateOnFocus: false,
  });
}
